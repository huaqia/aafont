...
public class StickerView extends FrameLayout {
    ...
    private final List<BitmapStickerIcon> drawable_sticker_icons = new ArrayList<>(4);
    private final List<BitmapStickerIcon> text_sticker_icons = new ArrayList<>(4);
    ...
    public void configDefaultIcons() {
        BitmapStickerIcon deleteIcon = new BitmapStickerIcon(
                ContextCompat.getDrawable(getContext(), R.drawable.ic_sticker_delete),
                BitmapStickerIcon.LEFT_TOP);
        deleteIcon.setIconEvent(new DeleteIconEvent());
        BitmapStickerIcon zoomIcon = new BitmapStickerIcon(
                ContextCompat.getDrawable(getContext(), R.drawable.ic_sticker_scale),
                BitmapStickerIcon.RIGHT_BOTOM);
        zoomIcon.setIconEvent(new ZoomIconEvent());
        BitmapStickerIcon flipIcon = new BitmapStickerIcon(
                ContextCompat.getDrawable(getContext(), R.drawable.ic_sticker_flip),
                BitmapStickerIcon.RIGHT_TOP);
        flipIcon.setIconEvent(new FlipHorizontallyEvent());

        drawable_sticker_icons.clear();
        drawable_sticker_icons.add(deleteIcon);
        drawable_sticker_icons.add(zoomIcon);
        drawable_sticker_icons.add(flipIcon);

        text_sticker_icons.clear();
        text_sticker_icons.add(deleteIcon);
        text_sticker_icons.add(zoomIcon);
        text_sticker_icons.add(flipIcon);
    }

    public void hideBorderIcons() {
        showBorder = false;
        showIcons = false;
    }
    /**
        ...
        if (handlingSticker != null && !locked && (showBorder || showIcons)) {
            ...
            if (showIcons) {
                ...
                if (handlingSticker.isDrawableSticker()) {
                    ...
                    // 文字sticker
                    for (int i = 0; i < text_sticker_icons.size(); i++) {
                        BitmapStickerIcon icon = text_sticker_icons.get(i);
                        switch (icon.getPosition()) {
                            case BitmapStickerIcon.LEFT_TOP:
                                configIconMatrix(icon, x1, y1, rotation);
                                break;
                            ...
                            case BitmapStickerIcon.RIGHT_BOTOM:
                                configIconMatrix(icon, x4, y4, rotation);
                                break;
                            case BitmapStickerIcon.RIGHT_MIDDLE:
        ...
        switch (currentMode) {
            ...
            case ActionMode.ZOOM_WITH_TWO_FINGER:
                if (handlingSticker != null) {
                    ...
                    if (handlingSticker.getWidth() * handlingSticker.getMatrixScale(moveMatrix) < handlingSticker.getMinWidth()
//                    ||handlingSticker.getWidth()*handlingSticker.getMatrixScale(moveMatrix)>=MAX_WIDTH
                    ) {
                        return;
                    }
                    handlingSticker.setMatrix(moveMatrix);
    ...
    public void zoomAndRotateSticker(@Nullable Sticker sticker, @NonNull MotionEvent event) {
        if (sticker != null) {
            ...
            moveMatrix.postScale(newDistance / oldDistance, newDistance / oldDistance, midPoint.x,
                    midPoint.y);
            moveMatrix.postRotate(newRotation - oldRotation, midPoint.x, midPoint.y);
            if (sticker.getWidth() * sticker.getMatrixScale(moveMatrix) < sticker.getMinWidth()
//                ||sticker.getWidth()*sticker.getMatrixScale(moveMatrix)>=MAX_WIDTH
            ) {
                return;
            }
            handlingSticker.setMatrix(moveMatrix);
    ...
    public void horScaleCurrentSticker(@NonNull MotionEvent event) {
        horScaleSticker(handlingSticker, event);
    }

    public void horScaleSticker(@Nullable Sticker sticker, @NonNull MotionEvent event) {
        if (sticker != null) {
            float newDistance = calculateDistance(midPoint.x, midPoint.y, event.getX(), event.getY());
            moveMatrix.set(downMatrix);
            moveMatrix.postScale(newDistance / oldDistance, newDistance / oldDistance, midPoint.x,
...
//                ||sticker.getWidth()*sticker.getMatrixScale(moveMatrix)>=MAX_WIDTH
    ...
    protected BitmapStickerIcon findCurrentIconTouched() {
        if (handlingSticker != null) {
            if (handlingSticker.isDrawableSticker()) {
                for (BitmapStickerIcon icon : drawable_sticker_icons) {
                ...
                for (BitmapStickerIcon icon : text_sticker_icons) {
                    float x = icon.getX() - downX;
                    float y = icon.getY() - downY;
                    float distance_pow_2 = x * x + y * y;
                    if (distance_pow_2 <= Math.pow(icon.getIconRadius() + icon.getIconRadius(), 2)) {
...
//        }
    ...
    public ArrayList<EffectData.StickerData> getStickerDatas() {
        if (stickers.size() > 0) {
            ArrayList<EffectData.StickerData> datas = new ArrayList<>();
            for (Sticker sticker : stickers) {
                float[] matrixValues = new float[9];
                sticker.getMatrix().getValues(matrixValues);
                datas.add(new EffectData.StickerData(((ImageSticker)sticker).getStickerName(), matrixValues));
            }
            return datas;
        } else {
            return null;
...
//  }
     ...
     * @param icons
     */
    public void setDrawableStickerIcons(@NonNull List<BitmapStickerIcon> icons) {
        this.drawable_sticker_icons.clear();
        this.drawable_sticker_icons.addAll(icons);
        invalidate();
    }
     ...
     * @param icons
     */
    public void setTextStickerIcons(@NonNull List<BitmapStickerIcon> icons) {
        this.text_sticker_icons.clear();
        this.text_sticker_icons.addAll(icons);
        invalidate();
    }
     ...
     * @return
     */
    private float getRotation(MotionEvent event, PointF pointF) {
        double x = (event.getX() - pointF.x);
        double y = (event.getY() - pointF.y);
        double radians = Math.atan2(y, x);
        return (float) Math.toDegrees(radians);
    }
    private float getRotationXX(float sx, float sy, PointF pointF) {
        double x = (sx - pointF.x);
        double y = (sy - pointF.y);
        double radians = Math.atan2(y, x);
        return (float) Math.toDegrees(radians);
    }
}